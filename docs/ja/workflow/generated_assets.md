...
| **1 回**のインポートに同名の `.sspj` が複数含まれる場合 | 出力フォルダが同じになるため、最初の 1 つだけを変換します。残りはインポート終了時に一覧で表示されます — どちらかをリネームするか、別々の出力フォルダにインポートしてください |
| `.sspj` から削除された画像・アニメパック | 出力フォルダに**古いファイルが残り、そのまま読み込めてしまいます** — 削除したアニメパックを参照しているシーンは、リソース欠落にはならず古い `.ssab` を再生し続けます。孤立した画像ともどもプロジェクトに同梱もされます。手動で削除してください |
