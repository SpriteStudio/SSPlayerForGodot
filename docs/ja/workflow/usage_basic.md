...
2. **エディタ上でのプレビュー**
   ノードを選択すると **SpriteStudio** ボトムパネルが表示されます。再生 / 逆再生 / 一時停止 / 停止のトランスポートとフレームスクラバを使って、**ゲームを実行せずにエディタ上でアニメーションを再生**できます。ショートカットは AnimationPlayer エディタと同じく **D** 再生 / **A** 逆再生 / **Shift+D** 先頭から / **S** 一時停止・停止（パネルがアクティブな時に有効）。
