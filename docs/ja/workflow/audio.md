...
> [!NOTE]
> サウンドは、このプレイヤーが共通ランタイムより一歩踏み込んでいる箇所のひとつです。`libssruntime` はサウンドキーを通過したことを *通知* するだけで、Flutter / React Native / wgpu の各プレイヤーはその通知をそのままアプリケーション側に渡します。`SpriteStudioPlayer2D` はそれに加えて再生機能そのものを内蔵しています（Godot には音を鳴らすために必要なものが既に揃っているためです）。Unity / Web / Ren'Py も各エンジンで同様に内蔵しています。
...
- **順再生時のみ鳴ります。** 実効的な再生方向が逆向きのとき（逆再生、ping-pong の復路）は何も鳴りません。これは Godot 固有ではなく[共通の制限](../limitations.md#再生機能の制約)です。負の `speed_scale` はこれに該当しません。逆再生になるのではなく停止し、停止した再生ヘッドはサウンドキーを通過しないため、やはり鳴りません。
- **シークは飛ばした分を再生しません。** 再生ヘッドをジャンプさせた場合、発火するのは到達フレームのイベントだけです。既に鳴っているサウンドが新しい位置へ再同期されることもありません。
- **音は、それを発火させた再生よりも長く生き残ります。** `pause()` / `stop()`、`set_animation()` による切り替えが終わらせるのは *アニメーション* であって音ではなく、既に鳴っているボイスは最後まで再生されます。これは全公式プレイヤー共通の方針であり、かつ守れる唯一の定義でもあります。voice を一時停止しても届くのは「すでに鳴っている音」だけで、保持中に通過するはずだったキーの音は鳴らず、`resume()` でも取り戻せないためです。また `loop_num` は再生回数であり無限指定が存在しないため、すべての音はいつか自然に終わります。アニメーションの尺より長く鳴る音があるなら、それはアニメーションデータ側の問題です。`play_audio` をオフにした場合とノードがツリーから外れた場合は停止します。
