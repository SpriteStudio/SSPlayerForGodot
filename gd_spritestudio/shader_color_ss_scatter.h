//static const String shader_color_ss_scatter = R"(
static const char* shader_color_ss_scatter = R"(
/*!
* \file		shader_color_ss_scatter.gdshader
...
vec4 ssPreProc( vec4 col, sampler2D tex, vec2 st, inout bool pass )
{
	...
	Coord = st;
	u = vec2( floor( Coord.x * A_TW ) / A_TW, floor( Coord.y * A_TH ) / A_TH );
	...
	vec2	v = vec2( sin( r ), cos( r ) ) * vec2( A_U1, A_V1 ) * 96.0 * fPower * r2;

//	vec4	Pixel = texture( tex, Coord + v );
	vec4	Pixel = ssTextureSample( tex, Coord + v );
